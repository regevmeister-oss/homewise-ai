...
export default function ResultPage() {
  ...
  const personalInsights = [
    `באזור שבחרת, משפחות עם ילדים צפויות להתמודד עם עלייה משמעותית בהוצאות חינוך, תחבורה ומחיה לאורך השנים.`,
    `לפי הסימולציה, המשכנתא שלך עלולה להגיע לשיעור גבוה מההכנסה בתוך 8–10 שנים אם ההכנסה לא תעלה בקצב דומה להוצאות.`,
    `רכישת בית זול יותר ב־300,000 ₪ יכולה לשפר משמעותית את רמת היציבות ולהקטין לחץ עתידי.`,
    `אם תגדיל/י את ההון העצמי לפני הקנייה, החופש הכלכלי שלך עשוי להשתפר בצורה משמעותית.`,
  ];
  ...
  const viralActions = [
    ...
    "השווה לחברים",
    "בדיקת זוגיות כלכלית",
    "מה הסיכוי שלך לשרוד משכנתא בישראל?"
  ];
          ...
          <div className="mt-12 grid gap-8 lg:grid-cols-2">
            ...
            <section className="rounded-3xl border border-white/10 bg-white/10 p-8">
              ...
              <div className="mt-10 grid gap-6 md:grid-cols-2">
                <Info title="תובנות AI אישיות" items={personalInsights} />
                <Info title="תובנות חכמות" items={insights} />
                <Info title="מה אפשר לשפר?" items={recommendations} />
                <Info title="Viral Loop" items={viralActions} />
              </div>
            </section>
          </div>
        </section>
